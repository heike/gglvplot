...
GeomLvplot <- ggplot2::ggproto("GeomLvplot", ggplot2::Geom,
  ...
  draw_group = function(data, panel_scales, coord,
    ...
    offset <- (i / (2 * data$k)) * width

    box <- data.frame(
      xmin = data$xmin + offset,
      xmax = data$xmax - offset,
      ymin = data$lv[[1]][,"lower"],
      ymax = data$lv[[1]][,"upper"],
      alpha = 0, #data$alpha,
      common,
      stringsAsFactors = FALSE
    )
...
#  required_aes = c("x", "lower", "upper", "middle", "ymin", "ymax")
  required_aes = c("x", "k") # if lv is part of this list, it fails because of missing value issues in draw_layer
)
